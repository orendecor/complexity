...
This makes $\Pi_i^p = CO-NP$, and because we know $NP=CO-NP$ we get $\Pi_i^p = \Sigma_i^p$.
\\
...
(x,y_1)\in L'
\end{align}
By definition $L'\in \Pi_{k-1}^p$ but we showed for all indexes $i<k$ that $\Pi_i^p = \Sigma_i^p$, and so we conclude $\Pi_{k-1}^p =  \Sigma_{k-1}^p$, and:
\begin{align*}
L'\in \Sigma_{k-1}^p
\end{align*}
...
\end{align*}
By induction we showed that for any $i\in \mathbb{N}$, $\Sigma_i^p\subseteq NP$ and this $PH\subseteq NP$, we know $NP\subseteq PH$, so we conclude: $PH= NP$.
\section*{Question 3}
